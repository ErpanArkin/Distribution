...
parser.add_argument("-s", "--sizes", help="three arguments in the order of number of data, \
					lower and higher bound of the sizes",
...
def randomword(length):
   return ''.join(random.choice(string.uppercase) for i in range(5))

with open(args.output_filename, 'w') as f:
    for i in range(int(args.sizes[0])):
        f.write("{} {}\n".format(randomword(3),np.random.randint(int(args.sizes[1]),high=int(args.sizes[2]))))