...
class files_distributor:
   ...
   def __init__(self,files_filename,nodes_filename):
       ...
       if self.nodes.empty:
          raise RuntimeError("Nodes are empty")      
   ...
   def distribute(self):
       ...
       while any(files['notAllocated']): #repeat if any of the files is not allocated.
           ...
           frest.sort_values('size',inplace=True,ascending=False)
   ...
   def plot_bar(self):
       ...
       files_nodes = pd.pivot_table(self.files[self.files['AssignedNode'] != 'NULL'], values='size', index='AssignedNode', columns='files')
       ...
       ax = total_fn.plot.bar(figsize=(5+n_col*2,5), stacked=True,title='Files distribution among nodes',cmap='nipy_spectral_r')
